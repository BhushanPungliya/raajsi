...
export default function RitualSlider() {
  const settings = {
    ...
    autoplay: false,
    customPaging: (i) => (
      <div className="w-[20px] h-[8px] bg-[#4D0B2F4d] rounded-full"></div>
    ),
    appendDots: dots => (
      <div className="!m-0">
        <ul className="flex gap-3 absolute lg:left-[50px] left-[50%] lg:translate-[0%] translate-[-50%]">{dots}</ul>
      </div>
    ),
  };
          ...
          <div key={idx}>
            <div className="rounded-t-[24px] overflow-hidden bg-[#FEFEFE] lg:px-[100px] px-[16px] lg:pt-[50px] pt-[26px] flex justify-between lg:flex-row flex-col-reverse lg:gap-0 gap-[50px]">
              <div className="max-w-[538px] w-full lg:pb-[234px] pb-[20px]">
                ...
                <p className="font-avenir-400 lg:text-[16px] text-[14px] text-left text-[#0C0C0C] lg:pb-[24px] pb-[20px]">
                  {item.desc}
                </p>
                <p className="font-avenir-400 lg:text-[20px] text-[14px] text-[#727272] text-left lg:pb-[45px] pb-[20px]">
                  {item.subText}
                </p>
                <Link href={item.link} className="flex items-start pb-[50px]">
